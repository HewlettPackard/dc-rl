...
class OnPolicyBaseRunner:
    ...
    def eval(self):
        ...
        # for agent_key, agent_data in metrics.items():
        #     # Assuming metric data is numeric and can be directly converted to numpy arrays
        #     for metric_key in agent_data[0].keys():
        #         metric_values = [data[metric_key] for data in agent_data if data[metric_key] is not None]
    ...
    #     """Dump collected metrics to a CSV file for each agent."""
    #     base_dir = os.path.join(self.run_dir, 'evaluation_data')
    #     os.makedirs(base_dir, exist_ok=True)
        
    #     for agent_key, data in metrics.items():
    #         filename = os.path.join(base_dir, f"{agent_key}_episode_{eval_episode}.csv")
    #         with open(filename, 'w', newline='') as file:
    #             writer = csv.DictWriter(file, fieldnames=data[0].keys())
    #             writer.writeheader()
    #             writer.writerows(data)
        
    #     print(f"Metrics for all agents have been saved to {base_dir}.")
