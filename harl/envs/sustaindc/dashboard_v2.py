...
class Dashboard:
    ...
    def setup_layout(self):
        ...
        default_experiment = experiments[0] if experiments else None
    ...
    def setup_callbacks(self):
        ...
        def update_graphs(selected_experiment, selected_episode, n_intervals):
            ...
            # **Second Column: Original Workload and Computed Workload**
            ...
            # Computed Workload trace
            context_fig_2.add_trace(go.Scatter(
                x=df_window['datetime'],
                y=df_window['ls_shifted_workload'] * 100,
                mode='lines',
                name='Computed Workload',
                line=dict(color='#F5D036', width=3)
            ))

            # Layout for the combined panel
            context_fig_2.update_layout(
                title=dict(text='Original and Computed Workload', font=dict(size=20)),
                yaxis_title=dict(text='Workload (%)', font=dict(size=18)),
                height=310,
            ...
            context_fig_2.update_yaxes(range=[0, 100])  # Set y-axis limits to 0-100%
            ...
            actions_fig_1 = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.2,
                                        subplot_titles=('Tasks Deferred/Processed', 'Tasks in Queue'))

            # Subplot 1: Tasks Deferred/Completed from Queue
            # Calculate net queue action: positive for added tasks, negative for removed tasks
            net_queue_action = (100 * (df_window['ls_shifted_workload'] - df_window['ls_original_workload'])).round(0)

            # Separate positive and negative actions
            positive_action_mask = net_queue_action > 0
            negative_action_mask = net_queue_action < 0

            # Positive actions: Increasing workload
            actions_fig_1.add_trace(
                go.Bar(
                    x=df_window['datetime'][positive_action_mask],
                    y=net_queue_action[positive_action_mask],
                    name='Processed',  # Positive actions
                    marker_color='green'
                ),
                row=1, col=1
            )

            # Negative actions: Removing workload
            actions_fig_1.add_trace(
                go.Bar(
                    x=df_window['datetime'][negative_action_mask],
                    y=net_queue_action[negative_action_mask],
                    name='Deferred',  # Negative actions
                    marker_color='red'
                ),
                row=1, col=1
            )

            actions_fig_1.update_yaxes(title_text='Tasks', row=1, col=1)
            ...
            # Update layout for the panel with adjusted legend position
            actions_fig_1.update_layout(
                annotations=[
                    dict(
                        text='Tasks Deferred/Processed',
                        font=dict(size=20),
                        x=0.5,
                        xref='paper',
                        y=1.05,
                ...
                font=dict(size=16),
                barmode='stack',  # Stack the bars for clear visualization
                bargap=0.15,
                bargroupgap=0.1,
                showlegend=True,
                legend=dict(
                ...
                height=400,
                margin=dict(l=100, r=20, t=40, b=50),  # Adjust margins as needed
            )
    ...
    def run_server(self, start_port=8074, max_attempts=10):
        port = start_port
        for attempt in range(max_attempts):
            try:
                print(f"Attempting to start server on port {port}...")
                self.app.run_server(debug=False, use_reloader=False, port=port)
                break  # Exit the loop if the server starts successfully
            except:
                print(f"Error starting server on port {port}")
