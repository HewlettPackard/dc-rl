...
pydcm_env = make_dc_pyeplus_env(month+1, ci_loc, max_bat_cap_Mw=2.0, use_ls_cpu_load=True) 
...
with tqdm(total=1000, desc="Processing", unit="iteration") as pbar:
    for _ in range(1000):
        dc_state, _, dc_terminated, dc_truncated, dc_info = pydcm_env.step(action)
        pbar.update(1)
