...
class myLinearRewardwCost(BaseReward):
    def __init__(
        self,
        env: Env,
        **kwargs
    ):
        super(myLinearRewardwCost, self).__init__(env)
        self.tou_prices = {0: 0.30, 
                            1: 0.30, 
                            2: 0.30, 
                            3: 0.30, 
                            4: 0.30, 
    ...
    def __call__(self) -> Tuple[float, Dict[str, Any]]:
        ...
        # Energy term mean and std [143527.3, 38481.004]
        # reward_energy = - (obs_dict['Facility Total Electricity Demand Rate(Whole Building)'] - 1.45e5) / (3.8e4)
        reward_energy = - (obs_dict['Facility Total Electricity Demand Rate(Whole Building)'] - 1.15e6) / (3.5e5)

        reward = reward_energy #* self.tou_prices[obs_dict['sinhour']]
        
        reward_terms = {
            'reward_energy': reward,
            'total_energy': obs_dict['Facility Total Electricity Demand Rate(Whole Building)'],
            'reward_comfort': 0,
            'abs_comfort': 0,
            'temperatures': 0
        }
