...
from custom_sinergym.utils.evaluation import evaluate_policy
from custom_sinergym.utils.wrappers import LoggerWrapper, NormalizeObservation
...
class LoggerCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if len(variables) > 1 and isinstance(action, list):
            for i, variable in enumerate(variables):
            ...
            if isinstance(action, list):
                self.record('action/' + variables[0], action[0])
            else:
                self.record('action/' + variables[0], action)
            if isinstance(action_, list):
                self.record('action_simulation/' + variables[0], action_[0])
            else:
                self.record('action_simulation/' + variables[0], action_)
