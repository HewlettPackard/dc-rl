...
class DataCenter_ITModel():
    ...
    def calculate_cooling_tower_water_usage_v2(self, Q_IT, T_hot, T_cold, T_wb, drift_rate=0.002, cycles_of_concentration=5, m_water_circulated=100):
        """
        Calculate the cooling tower water usage in liters per 15 minutes, incorporating temperature range.

        Parameters:
        Q_IT (float): Heat generated by the IT equipment in Watts (W).
        drift_rate (float): Drift rate as a fraction of water circulated.
        cycles_of_concentration (float): The number of cycles of concentration in the cooling tower.
        m_water_circulated (float): Mass flow rate of water circulated in the cooling tower in kg/s.
        T_hot (float): Hot water temperature returning to the cooling tower in Celsius (°C).
        T_cold (float): Cold water temperature leaving the cooling tower in Celsius (°C).
        T_wb (float): Ambient wet-bulb temperature in Celsius (°C).

        Returns:
        float: Cooling tower water usage in liters per 15 minutes.
        """
        # Constants
        h_fg = 2260 * 1e3  # Latent heat of vaporization in J/kg
        rho_water = 1000  # Density of water in kg/m^3
        Cp = 4184  # Specific heat capacity of water in J/kg·K

        # Calculate the total heat load based on temperature difference
        Q_total = m_water_circulated * Cp * (T_hot - T_cold)

        # Adjust the total heat load for ambient conditions
        def f(T_wb):
            # Example function for wet-bulb temperature adjustment; refine based on real data
            return 1 - 0.005 * (T_wb - 27)

        Q_total_adj = Q_total * f(T_wb) + Q_IT

        # Calculate evaporative losses
        m_evap = Q_total_adj / h_fg  # kg/s

        # Calculate drift losses
        m_drift = drift_rate * m_water_circulated  # kg/s

        # Calculate blowdown
        m_blowdown = m_evap / (cycles_of_concentration - 1)  # kg/s

        # Total mass of water lost
        m_total = m_evap + m_drift + m_blowdown  # kg/s

        # Convert to volumetric flow rate (liters per second)
        V_total = m_total / rho_water  # m^3/s
        V_total_liters_per_sec = V_total * 1000  # liters per second

        # Convert to liters per 15 minutes
        V_total_liters_per_15min = V_total_liters_per_sec * 15 * 60  # liters per 15 minutes

        return round(V_total_liters_per_15min, 4)
